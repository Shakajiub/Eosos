...
AStar::AStar() : path_found(false), path_marker(nullptr), follow_actor(nullptr)
{
...
bool AStar::find_path(Dungeon *scene, int8_t start_x, int8_t start_y, int8_t end_x, int8_t end_y)
{
	// Make sure we're not trying to path into a wall
	if (scene == nullptr || scene->get_level()->get_wall(end_x, end_y) || (end_x == start_x && end_y == start_y))
		return false;

	if (scene->get_level()->get_actor(end_x, end_y) != nullptr)
		follow_actor = scene->get_level()->get_actor(end_x, end_y);
	...
	while (!stop_search && !open_list.empty())
	{
		...
		for (uint8_t i = 0; i < 8; i++)
		{
			...
			// TODO - Can't path through impassable walls
			/*if ((new_x != end_x && new_y != end_y) &&
				scene->get_level()->get_object(new_x, new_y) != nullptr &&
				scene->get_level()->get_object(new_x, new_y)->get_passable())
				continue;*/
...
void AStar::step(Dungeon *scene)
{
	...
	{
		if (get_last_x() != follow_actor->get_grid_x() || get_last_y() != follow_actor->get_grid_y())
			find_path(scene, current_x, current_y, follow_actor->get_grid_x(), follow_actor->get_grid_y());
	}
}
