...
AbilityPoison::AbilityPoison() : target_texture(nullptr), temp_hero(nullptr)
{
	valid_nodes.clear();
	ability_desc = "Poison a nearby creature";
}
AbilityPoison::~AbilityPoison()
{
	free();
}
void AbilityPoison::free()
{
...
{
	if (init_texture("core/texture/ui/icon/poison.png", COLOR_LEAF))
		ability_name = "poison";
...
void AbilityPoison::render(uint16_t xpos, uint16_t ypos, SDL_Keycode key) const
{
...
bool AbilityPoison::get_click(uint16_t mouse_x, uint16_t mouse_y)
{
	...
	for (auto node : valid_nodes)
	{
		if (node.xpos == map_x && node.ypos == map_y)
		{
			...
			{
				node.target->set_status(STATUS_POISON);
			}
			clear(temp_hero);
			return true;
		}
...
void AbilityPoison::clear(Hero *hero)
{
	valid_nodes.clear();
